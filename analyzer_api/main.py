...
async def save_to_es(data: dict):
    try:
        ...
        for doc in data["dataframe"]:
            formatted_doc = convert_data_types(doc)  # ✅ Veri formatını düzelt
...
async def delete_file(filename:str):
    try:
        ...
        if(filename in os.listdir(UPLOAD_DIR)):
            ...
            return {"message":f"belirtilen dosya bulunamadi"}
    except Exception as e:
...
async def get_time_based_stats_by_endpoint(index: str, endpoint: str):
    try:
        query = {
            "size": 0,
            "query": {
        ...
        response = requests.get(url=f'{ELASTICSEARCH_URL}/{index}/_search',
                                headers={"Content-Type": "application/json"},
                                data=query_json)
        ...
        if "aggregations" in data:
            # Assuming there is at least one date bucket
            bucket = data["aggregations"]["zaman_serisi"]["buckets"][0]
